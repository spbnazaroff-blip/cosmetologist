...
$schemas=[
 'services'=>['label'=>'Услуги и цены','fields'=>['title'=>'Название','subtitle'=>'Подзаголовок','category'=>'Категория','price'=>'Цена','duration'=>'Длительность','summary'=>'Короткое описание','body'=>'Полное описание']],
 'articles'=>['label'=>'Статьи блога','fields'=>['title'=>'Заголовок','category'=>'Категория','date'=>'Дата','read_time'=>'Время чтения','excerpt'=>'Анонс','body'=>'Текст статьи']],
 'cases'=>['label'=>'Результаты / кейсы','fields'=>['title'=>'Название кейса','period'=>'Период','request'=>'Исходный запрос','protocol'=>'Что делали','result'=>'Результат','before_image'=>'Фото до (путь/URL)','after_image'=>'Фото после (путь/URL)']],
 'videos'=>['label'=>'Видео','fields'=>['title'=>'Название','date'=>'Дата','url'=>'Ссылка VK Видео или RuTube','description'=>'Описание']],
];
    ...
    if(!csrf_ok()){$error='Сессия формы устарела. Обновите страницу.';}
      ...
      if(($_POST['action']??'')==='delete'){$items=array_values(array_filter($items,fn($i)=>(string)($i['id']??'')!==$id));$message=save_content($type,$items)?'Запись удалена.':'Не удалось сохранить изменения.';}
        ...
        if($error===''){
          ...
          if(!$found)$items[]=$row;
          if(save_content($type,$items)){$message='Сохранено.';}else{$error='Не удалось записать JSON. Проверьте права папки storage.';}
        }
      }
    }
}
...
:root{--bg:#f3efe9;--paper:#fff;--ink:#1d1916;--muted:#746d66;--line:#ddd4ca;--dark:#181411;--accent:#9b775d}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font:14px/1.5 Arial,sans-serif}a{color:inherit;text-decoration:none}.wrap{width:min(1180px,calc(100% - 28px));margin:35px auto}.auth{max-width:460px;margin:10vh auto;padding:32px;background:#fff;border:1px solid var(--line);border-radius:20px}.auth h1{font:32px Georgia,serif}.field{display:grid;gap:6px;margin:14px 0}.field span{font-size:11px;color:var(--muted)}input,textarea{width:100%;padding:11px 12px;border:1px solid var(--line);border-radius:10px;background:#fff}textarea{min-height:105px;resize:vertical}.btn{display:inline-flex;justify-content:center;align-items:center;border:0;border-radius:999px;padding:11px 17px;background:var(--dark);color:#fff;cursor:pointer}.btn.alt{background:#e9e1d8;color:#2c251f}.btn.danger{background:#7e312b}.notice{padding:12px 14px;border-radius:10px;margin:12px 0;background:#e6f2e8}.error{background:#f5dfdd}.admin-head{display:flex;justify-content:space-between;gap:20px;align-items:center;margin-bottom:24px}.admin-head h1{margin:0;font:34px Georgia,serif}.tabs{display:flex;gap:7px;flex-wrap:wrap;margin-bottom:22px}.tabs a{padding:9px 13px;border:1px solid var(--line);border-radius:999px}.tabs a.active{background:var(--dark);color:#fff}.grid{display:grid;grid-template-columns:1fr 390px;gap:22px}.panel{background:#fff;border:1px solid var(--line);border-radius:18px;padding:20px}.rows{display:grid;gap:9px}.row{display:grid;grid-template-columns:1fr auto;gap:15px;align-items:center;padding:13px;border:1px solid var(--line);border-radius:12px}.row strong{display:block}.row small{color:var(--muted)}.form-actions{display:flex;gap:8px;margin-top:18px;flex-wrap:wrap}.check{display:flex;gap:8px;align-items:center}.check input{width:auto}@media(max-width:850px){.grid{grid-template-columns:1fr}.admin-head{align-items:flex-start;flex-direction:column}.wrap{margin-top:18px}}
</style></head><body>
...
<?php else:?><div class="wrap"><header class="admin-head"><div><small>ÉLAN SKIN · управление контентом</small><h1><?=aesc((string)$schemas[$type]['label'])?></h1></div><div><a class="btn alt" href="/" target="_blank">Открыть сайт ↗</a> <a class="btn alt" href="?logout=1">Выйти</a></div></header>
<nav class="tabs"><?php foreach($schemas as $key=>$schema):?><a class="<?=$key===$type?'active':''?>" href="?type=<?=$key?>"><?=aesc((string)$schema['label'])?></a><?php endforeach;?></nav>
<?php if($message):?><div class="notice"><?=aesc($message)?></div><?php endif;?><?php if($error):?><div class="notice error"><?=aesc($error)?></div><?php endif;?>
<div class="grid"><section class="panel"><div class="rows"><?php foreach(load_content($type) as $item):?><div class="row"><div><strong><?=aesc((string)($item['title']??'Без названия'))?></strong><small><?=aesc((string)($item['price']??$item['date']??$item['period']??''))?><?=$item['featured']??false?' · на главной':''?></small></div><a class="btn alt" href="?type=<?=aesc($type)?>&edit=<?=rawurlencode((string)$item['id'])?>">Изменить</a></div><?php endforeach;?></div></section>
<aside class="panel"><h2 style="font:26px Georgia,serif;margin-top:0"><?=$edit?'Редактирование':'Новая запись'?></h2><form method="post"><input type="hidden" name="csrf" value="<?=aesc(csrf())?>"><input type="hidden" name="action" value="save"><input type="hidden" name="id" value="<?=aesc((string)($edit['id']??''))?>"><?php if(in_array($type,['services','articles','cases'],true)):?><label class="field"><span>Slug / адрес страницы</span><input name="slug" value="<?=aesc((string)($edit['slug']??''))?>" placeholder="создастся автоматически"></label><?php endif;?><?php foreach($schemas[$type]['fields'] as $field=>$label):?><label class="field"><span><?=aesc((string)$label)?></span><?php if(in_array($field,['body','excerpt','summary','request','protocol','result','description'],true)):?><textarea name="<?=aesc($field)?>"><?=aesc((string)($edit[$field]??''))?></textarea><?php else:?><input <?= $field==='date'?'type="date"':'' ?> name="<?=aesc($field)?>" value="<?=aesc((string)($edit[$field]??($field==='date'?date('Y-m-d'):'')))?>"><?php endif;?></label><?php endforeach;?><label class="check"><input type="checkbox" name="featured" value="1" <?=!empty($edit['featured'])?'checked':''?>> Показывать среди избранного на главной</label><div class="form-actions"><button class="btn" type="submit">Сохранить</button><a class="btn alt" href="?type=<?=aesc($type)?>">Очистить форму</a></div></form><?php if($edit):?><form method="post" onsubmit="return confirm('Удалить запись?')" style="margin-top:10px"><input type="hidden" name="csrf" value="<?=aesc(csrf())?>"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?=aesc((string)$edit['id'])?>"><button class="btn danger" type="submit">Удалить</button></form><?php endif;?></aside></div></div><?php endif;?></body></html>